class LikeTagAndFeed {
  constructor(props) {
    ...
    this.deslikeText = props.deslikeText;
  ...
  like(link, idx) {
    ...
    setTimeout(() => {
      ...
      const likeButton = this.getLikeButton(modal);

      if(likeButton) {
        if (this.getLikeButtonText(likeButton) === this.deslikeText) {
...
new LikeTagAndFeed({
  ...
  getLikeButton: modal => modal.children[2].firstChild.firstChild.firstChild,
